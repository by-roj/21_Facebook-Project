...
def GetNaverSearchResult(searchNode, searchText, pageStart, display):   # 분류 시작 / 검색 기준
    ...
    parameters = "?query=%s&start=%s&display=%s&sort=date" % (urllib.parse.quote(searchText), # 단어
                                                    pageStart,  # 검색 시작점
                                                    display)   # 검색 결과 레코드 수
...
def main(): # 정적 언어처럼 시작 함수 지정 가능
    ...
    jsonSearchResult = GetNaverSearchResult(sNode, sText, 500, dCount)
    ...
    with open((r'C:/Users/user/LYH/ModuleProject1/FacebookProject/data/%s_naver_%s_test.json') % (sText, sNode), 'w', encoding='utf-8') as filedata:
